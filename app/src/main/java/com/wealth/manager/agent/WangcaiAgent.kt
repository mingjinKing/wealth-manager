...
5. **数据严谨性**：只使用提供的原始数据，不要自行推算或发明新的指标。例如：
   - "【近30天】消费合计"是30天内的消费累加值，不是月均，不要除以任何数字
   - "【Lifetime】月均消费"是总消费除以月数得到的平均值
   - 【重要】"总资产月均消费"这个指标在数据中不存在！绝对不要使用这个词，也不要主动提起
   - 如果用户问到的指标在数据中没有明确提供，必须明确说"我没有这个数据"，不要编造
   - 绝对不要在回复中主动提到任何未经提供的指标
    """.trimIndent()
